...
fn make_url(full_path: &str, _root_path: &str) -> String {
    ...
    if let Some(colon_idx) = clean_path.find(':') {
        if colon_idx == 1 {
            ...
            let encoded_parts: Vec<String> = path_after_drive
                .split('/')
...
pub fn PageNotFound(route: Vec<String>) -> Element {
    rsx! { div { class: "container", h1 { "404 - Page non trouvée" }, Link { to: Route::Home {}, class: "btn-nav", "Retour Accueil" } } }
}
...
pub fn Settings() -> Element {
    ...
    let mut sources_signal = use_signal(|| {
        ...
        for s in config.video_sources {
            paths.push(s.path.to_string_lossy().to_string());
        }
        for s in config.music_sources {
            paths.push(s.path.to_string_lossy().to_string());
        }
        for s in config.image_sources {
            paths.push(s.path.to_string_lossy().to_string());
        }
        paths.sort();
        paths.dedup();
